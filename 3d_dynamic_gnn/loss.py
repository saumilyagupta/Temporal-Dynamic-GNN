...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha: float = 0.5, gamma: float = 1.0, reduction: str = 'mean'):
        """
        Args:
            alpha: Balancing factor for class weights (typically num_normal / (num_normal + num_anomalous))
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        # For binary classification: p_t = prob[class] where class is the target
        batch_size = targets.size(0)
        p_t = probs.gather(1, targets.unsqueeze(1)).squeeze(1)  # [B]
        
        # Clamp p_t to avoid numerical instability (log(0) = -inf)
        ...
        if torch.isnan(focal_loss).any() or torch.isinf(focal_loss).any():
            # Fallback to standard cross-entropy if focal loss has issues
