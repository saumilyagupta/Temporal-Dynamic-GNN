"""
Train TemporalGNN on CarCrash with NiAD-style graphs (8-D from ``det`` bboxes, no YOLO / no VGG).
"""
...
from dataset_carcrash_niad import (
    create_carcrash_niad_dataloaders,
    create_carcrash_niad_precomputed_dataloaders,
)
from evaluate_carcrash_niad import evaluate_and_save
...
def main():
    parser = argparse.ArgumentParser(description="TemporalGNN — CarCrash NiAD-style (det-only)")
    parser.add_argument(
        "--config",
        type=str,
        default="config_carcrash_niad.yaml",
    ...
    base_name = args.exp_name or "carcrash_niad_temporal_gnn"
    exp_dir = os.path.join("experiments", f"{base_name}_{ts}")
    os.makedirs(exp_dir, exist_ok=True)

    print("\n" + "=" * 60)
    print("CarCrash TemporalGNN — NiAD-style (det-only, 8-D nodes)")
    print("=" * 60)
    ...
    if data_cfg.get("source", "npz") == "precomputed":
        ...
        train_loader, val_loader, test_loader, train_dataset = create_carcrash_niad_dataloaders(
    ...
    if best_path:
        ...
        print("\nRunning CarCrash NiAD evaluation (combined + Day/Night)...")
        evaluate_and_save(
    ...
    with open(summary_path, "w") as f:
        f.write("CarCrash TemporalGNN (NiAD-style det) — summary\n")
