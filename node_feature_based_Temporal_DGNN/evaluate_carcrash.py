"""
Post-training evaluation for CarCrash: combined + Day/Night subsets, file outputs.
"""
...
def load_crash_timing(annotation_file: str) -> Dict[str, str]:
    """
    Parse Crash-1500.txt: vidname -> 'Day' or 'Night'.
    """
    timing: Dict[str, str] = {}
...
def _subset_mask(
    ...
    """
    Build boolean mask for test rows.

    subset: 'combined' | 'day' | 'night'
    Day: all normal + crash videos annotated as Day
    Night: all normal + crash videos annotated as Night
    """
    n = len(labels)
...
def evaluate_and_save(
    ...
    """
    Run test evaluation with Day/Night breakdown; save under exp_dir/results/.
    """
    ...
    root = data_cfg.get("root_dir", data_cfg.get("graph_export_root", ""))
    ...
    if data_cfg.get("source", "npz") == "precomputed":
        ...
        if val_split > 0.0 and val_split < 1.0 and len(train_entries) > 1:
            ...
            train_only = train_entries

        train_ds = CarCrashPrecomputedDataset(
            train_only,
            num_frames=num_frames,
            normalize_features=norm,
        )
        stats = train_ds.get_normalization_stats() if norm else None
        test_ds = CarCrashPrecomputedDataset(
            test_entries,
            num_frames=num_frames,
            normalize_features=norm,
            feature_stats=stats,
    ...
    with open(summary_path, "w") as f:
        f.write("CarCrash TemporalGNN — Evaluation summary\n")
        f.write("=" * 60 + "\n")
    ...
    print(f"\n[evaluate_carcrash] Results saved under: {results_dir}")
...
def main():
    ...
    p.add_argument("--config", type=str, default="config_carcrash.yaml")
